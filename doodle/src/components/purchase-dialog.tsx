...
export function PurchaseDialog({ open, account, scene, locale, copy, success, errorMessage, confirmationBusy = false, onRetryConfirmation, onRestoreFocus, onClose, onAccountChange, onExpressCheckoutComplete, signInOnly = false }: PurchaseDialogProps) {
  ...
  const saveReturn = useCallback((intent: ReturnIntent, preserveExisting = false) => {
    try {
      if (preserveExisting && sessionStorage.getItem("doodle:return")) return;
      sessionStorage.setItem("doodle:return", JSON.stringify({ scene, intent, path: location.pathname }));
    } catch {}
  }, [scene]);
  ...
  useEffect(() => {
    ...
    setDialogReady(true);
    return () => { setDialogReady(false); if (dialog.open && typeof dialog.close === "function") dialog.close(); if (onRestoreFocus) onRestoreFocus(); else previousFocus.current?.focus(); previousFocus.current = null; };
  }, [onRestoreFocus, open]);
  ...
  useEffect(() => {
    if (!open || playRuntime || step !== "signIn") return;
    saveReturn(signInOnly ? "account" : "auth", signInOnly);
  }, [open, playRuntime, saveReturn, signInOnly, step]);
