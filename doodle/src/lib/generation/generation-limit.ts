...
const NATIVE_CHALLENGE_CLIENT_DAILY_LIMIT = 20;
const NATIVE_CHALLENGE_GLOBAL_DAILY_LIMIT = 200;
...
export type NativeChallengeOperation = "guest" | "generate";
...
export async function checkNativeChallengeLimit(
  request: Request,
  operation: NativeChallengeOperation,
  ...
  const ip = request.headers.get("x-vercel-forwarded-for") ?? request.headers.get("x-forwarded-for") ?? "unknown";
  const client = createHmac("sha256", secret).update(`native-challenge:${operation}:${ip}`).digest("hex");
  const day = new Date().toISOString().slice(0, 10);
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify([
        ...
        COUNT_SCRIPT,
        "2",
        `doodle:native:challenge:client:${day}:${operation}:${client}`,
        `doodle:native:challenge:global:${day}`,
        NATIVE_CHALLENGE_CLIENT_DAILY_LIMIT,
        NATIVE_CHALLENGE_GLOBAL_DAILY_LIMIT,
        COUNTER_TTL_SECONDS,
      ]),
      cache: "no-store",
    });
    ...
    return clientCount > NATIVE_CHALLENGE_CLIENT_DAILY_LIMIT || globalCount > NATIVE_CHALLENGE_GLOBAL_DAILY_LIMIT
      ? "rate_limited"
      : "allowed";
  } catch {
    return "unavailable";
